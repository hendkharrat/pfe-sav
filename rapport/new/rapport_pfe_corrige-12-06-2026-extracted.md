...
Application Web de Gestion du Service Après-Vente (SAV Manag)

---

## Dédicace

[IMAGE À INSÉRER : page de dédicace]

---

## Remerciements

[IMAGE À INSÉRER : page de remerciements]

---

## Table des matières

[IMAGE À INSÉRER : table des matières]

---

## Table des figures

[IMAGE À INSÉRER : table des figures]

---

## Liste des tableaux

[IMAGE À INSÉRER : liste des tableaux]
...
W hedheka star eli fouk kol page ykolek enty win
...
**Couche logique métier (Backend / API)** La couche backend expose des routes API RESTful implémentées sous forme de contrôleurs. Chaque contrôleur gère la validation des données, l'exécution de la logique métier et les appels vers la couche d'accès aux données.

**Couche d'accès aux données (DAO / Repository)** L'accès à la base de données est centralisé dans une couche DAO (Data Access Object) utilisant Prisma comme ORM. Cette couche exécute les requêtes vers la base de données MySQL et garantit l'intégrité des données via la gestion des transactions (COMMIT/ROLLBACK).
...
Nom : S'authentifier Acteur principal : Tout utilisateur (Admin / Technicien / Client) Participants : Interface (Frontend) → Contrôleur (AuthController) → DAO (UserDAO / ClientDAO) → Base de données (MySQL)
...
- Le DAO retourne un message d'erreur au Contrôleur.
...
- Le DAO détecte que le technicien a déjà une intervention à cette date.
...
- Le DAO détecte que le technicien est inexistant ou inactif.
...
- Le DAO détecte que le technicien est indisponible à la date prévue.
- L'Interface affiche « Technicien indisponible à la date prévue ».
