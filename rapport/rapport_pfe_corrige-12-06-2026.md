...
Les entreprises sont aujourd'hui confrontées à une transformation digitale accélérée, les obligeant à repenser leurs processus pour améliorer leur efficacité, leur réactivité et la qualité de leurs services. Cette évolution numérique a un impact majeur sur les systèmes de gestion interne, où la centralisation et l'automatisation des données sont devenues des clés de la performance. Dans ce contexte, les données jouent un rôle stratégique dans la prise de décision. Elles permettent de suivre les activités, d'anticiper les besoins, d'optimiser les ressources et de garantir une traçabilité fiable des opérations. La mise en place de systèmes d'information structurés est donc indispensable, en particulier dans les domaines qui nécessitent une forte coordination, comme le service après-vente (SAV).

Le service après-vente est un élément essentiel pour les entreprises spécialisées dans l'installation et la maintenance d'équipements techniques. Il englobe des activités telles que la gestion des clients, des contrats, des équipements, ainsi que la planification et le suivi des interventions préventives et curatives. Cependant, ces processus sont souvent gérés de manière manuelle ou fragmentée, ce qui entraîne des difficultés de suivi, des pertes d'information et un manque de visibilité globale.

Ce projet de fin d'études propose la conception et la réalisation d'une application web de gestion du service après-vente pour la maintenance des climatiseurs et des systèmes de surpression. L'objectif principal est de centraliser les données, d'automatiser les processus métier et d'améliorer la coordination entre les différents acteurs du système.

Pour atteindre ces objectifs, la méthodologie Scrum a été adoptée. Cette approche agile permet un développement itératif et incrémental du système à travers plusieurs sprints, tout en assurant une meilleure adaptation aux besoins et une amélioration continue du produit.

Notre travail sera structuré de la façon suivante :
...
L'organisation de l'entreprise repose sur une structure orientée projet, où les équipes travaillent de manière collaborative. L'environnement de travail est basé sur des outils modernes de développement logiciel, favorisant les méthodologies agiles, notamment Scrum, ainsi que l'utilisation de technologies web récentes comme React et Next.js.
...
La contribution principale a consisté à concevoir et développer une application web de gestion du service après-vente. Ce travail a permis de structurer les processus de gestion des interventions, d'automatiser la planification des opérations de maintenance, d'améliorer la traçabilité des interventions et de participer à la modélisation et à la réalisation des différents modules du système.
...
### 1.3.3 Les applications existantes
...
L'analyse SWOT de la situation actuelle du service après-vente montre que l'organisation a déjà une structure en place, mais qu'elle n'est pas suffisamment moderne. Le système actuel repose sur des méthodes qui fonctionnent déjà, comme la gestion des clients, des équipements et des contrats de maintenance, ainsi que sur les informations sur les interventions passées. Ces éléments sont très importants pour créer un système informatique. Cependant, cette analyse met également en évidence plusieurs problèmes importants, notamment l'absence de centralisation et le manque d'automatisation des processus de facturation et de planification.
...
Pour ce projet, la méthodologie Scrum a été choisie. Cette méthode repose sur des cycles de développement courts appelés Sprints, une liste de fonctionnalités appelée Product Backlog et une planification des tâches pour chaque sprint. Le projet est divisé en plusieurs sprints, chacun permettant de livrer une partie fonctionnelle du système. Les principaux avantages de Scrum sont une meilleure organisation du travail, une visibilité continue de l'avancement, une adaptation rapide aux besoins et une amélioration continue.
...
- L'application doit permettre à tout utilisateur de se connecter à l'aide de son adresse e-mail **ou** de son numéro de téléphone à 8 chiffres (sans indicatif pays), accompagné de son mot de passe.
- L'accès aux fonctionnalités est restreint selon le rôle de l'utilisateur (administrateur, technicien, client).
- Tout utilisateur connecté peut se déconnecter et fermer sa session.

**Gestion des utilisateurs internes**
- L'administrateur peut créer, consulter, modifier et désactiver les comptes des utilisateurs internes (administrateurs et techniciens).
- La désactivation est logique : l'attribut `actif` passe à `false`. L'utilisateur désactivé reste visible dans la liste avec une action « Restaurer » permettant de réactiver son accès.

**Gestion des clients**
- L'administrateur peut créer, consulter, modifier et supprimer les fiches clients.
- Un client peut être de type **société** (`SOCIETE`) ou **personne physique** (`PERSONNE_PHYSIQUE`).
- La ville du client est sélectionnée depuis une liste de villes tunisiennes.
- Chaque client dispose d'un mot de passe pour l'accès au portail client.

**Catalogue d'équipements**
- L'administrateur gère un catalogue global d'équipements, indépendant de tout client.
- Chaque équipement est de type **climatiseur** (`CLIMATISEUR`) ou **système de surpression** (`SYSTEME_SURPRESSION`).
- Un équipement peut posséder plusieurs images (`EquipmentImage`), dont une est désignée comme image principale.

**Affectation équipement-client (ClientEquipement)**
- L'administrateur affecte un équipement du catalogue à un client en créant un enregistrement `ClientEquipement`, contenant la date d'achat, la date d'installation et une localisation optionnelle.
- L'affectation peut être initiée depuis le module client ou depuis la fiche équipement.

**Gestion des contrats**
- L'administrateur peut créer, consulter, modifier et supprimer des contrats de maintenance.
- Un contrat est associé à un client et couvre un ensemble d'**installations `ClientEquipement`** de ce client.
- La périodicité est choisie parmi : mensuelle, trimestrielle, semestrielle, annuelle.
- Le statut du contrat (`ACTIF`, `BIENTOT_EXPIRE`, `EXPIRE`) est calculé automatiquement par le système à partir des dates.

**Génération automatique du planning préventif**
- Lors de la création d'un contrat, le **système** génère automatiquement les interventions préventives selon la périodicité choisie et les installations `ClientEquipement` couvertes.
- Une prévisualisation du planning généré est affichée avant validation du contrat.

**Gestion des interventions**
- L'application gère les interventions préventives et curatives.
- Aucun champ de priorité n'est associé aux interventions.
- Avant d'affecter un technicien, le système vérifie sa disponibilité : un technicien ne peut pas être affecté à deux interventions prévues à la même date.

**Déclaration de pannes**
- Le client peut déclarer une panne sur l'un de ses équipements affectés, en saisissant une description obligatoire et en joignant plusieurs pièces jointes (images, PDF, audio).
- Aucun champ de priorité n'est associé à la panne.
- L'administrateur peut prendre en charge une panne et la convertir en intervention curative.

**Facturation**
- Une facture est générée uniquement pour une intervention de type **curative**, dont le statut est **réalisée** et dont la couverture contractuelle est **absente** (`couvertureContrat = false`).
- Les interventions préventives ne génèrent jamais de facture.
- La facture inclut des lignes de détail (`LigneFacture`), un montant HT, une TVA de **19 %** et un montant TTC en **Dinar Tunisien (TND)**.
- L'administrateur peut marquer une facture comme payée.

**Tableaux de bord différenciés**
- L'administrateur dispose d'un tableau de bord global avec KPIs et graphiques d'activité.
- Le technicien dispose d'un tableau de bord personnel avec ses interventions assignées et son planning.
- Le client dispose d'un espace personnel avec ses équipements, interventions, pannes et factures.

**Historique et planning**
- Chaque acteur dispose d'un historique filtré selon son rôle.
- Le planning est consultable en vue hebdomadaire (1 semaine, 2 semaines) et mensuelle.
...
[Insérer ici le diagramme de cas d'utilisation global — 4 acteurs : Administrateur, Technicien, Client, Système]
...
[Insérer ici le diagramme de classes global]
...
Le frontend de l'application est développé en utilisant Next.js et React. Cette combinaison permet de créer une interface utilisateur à la fois moderne et dynamique. Les composants React assurent une séparation claire des responsabilités d'affichage.

**Couche logique métier (Backend / API)**
La couche backend expose des routes API RESTful implémentées sous forme de contrôleurs. Chaque contrôleur gère la validation des données, l'exécution de la logique métier et les appels vers la couche d'accès aux données.

**Couche d'accès aux données (DAO / Repository)**
L'accès à la base de données est centralisé dans une couche DAO (Data Access Object) utilisant Prisma comme ORM. Cette couche exécute les requêtes vers la base de données MySQL et garantit l'intégrité des données via la gestion des transactions (COMMIT/ROLLBACK).

[Insérer ici le diagramme d'architecture 3 couches]

## Conclusion du Chapitre 2

Dans ce chapitre, nous avons défini les besoins fonctionnels et non fonctionnels de l'application avec leurs règles métier précises. Nous avons identifié quatre acteurs (Administrateur, Technicien, Client, Système) et modélisé leurs interactions au travers de 28 User Stories réparties en 3 sprints. Le diagramme de classes global introduit les 11 entités du modèle, notamment l'entité pivot `ClientEquipement` qui lie les clients aux équipements du catalogue. Le prochain chapitre sera consacré au Sprint 1 — Authentification et référentiel métier.
...
Dans le chapitre précédent, nous avons abordé la phase de préparation du projet et les exigences du système. Ce chapitre se concentre sur le premier sprint du projet, mené selon la méthodologie Scrum. Ce sprint est crucial pour le développement de l'application, car il pose les bases du système, notamment en termes d'authentification des utilisateurs et de gestion des données métier clés.
...
7. Le DAO insère l'enregistrement dans la table `ClientEquipement`.
...
6. Le DAO insère le contrat avec la liste `clientEquipementIds[]`.
...
Nom : S'authentifier
Acteur principal : Tout utilisateur (Admin / Technicien / Client)
Participants : Interface (Frontend) → Contrôleur (AuthController) → DAO (UserDAO / ClientDAO) → Base de données (MySQL)
...
4. Le DAO interroge la base de données.
...
Scénarios alternatifs :
- SA1 : Identifiant non reconnu → le DAO retourne null → erreur « Identifiant incorrect ».
...
Nom du cas d'utilisation : Créer un utilisateur
Acteur principal : Administrateur
Objectif : Permettre à l'administrateur d'ajouter un nouvel utilisateur interne dans le système avec un rôle spécifique.
...
5. Le DAO interroge la base de données.
...
9. Le DAO insère les données dans la base de données.
...
1. La base de données indique que l'e-mail existe déjà.
2. Le Contrôleur envoie un message d'erreur à l'Interface.
3. L'Interface affiche le message « E-mail déjà utilisé ».

Scénario alternatif 2 — Erreur base de données :
1. Une erreur survient pendant l'insertion.
2. Le DAO retourne un message d'erreur au Contrôleur.
3. L'Interface affiche un message d'erreur à l'administrateur.
...
Acteur principal : Administrateur
Objectif : Permettre à l'administrateur de retirer l'accès d'un utilisateur sans supprimer définitivement ses données (soft-delete).
...
4. Le Contrôleur demande la désactivation au DAO.
5. Le DAO effectue une mise à jour dans la base de données : `actif = false`.
...
1. Une erreur survient lors de la mise à jour.
2. Le DAO retourne un message d'erreur au Contrôleur.
3. L'Interface affiche un message d'erreur à l'administrateur.
...
Nom du cas d'utilisation : Créer un client
Acteur principal : Administrateur
Objectif : Permettre à l'administrateur d'ajouter un nouveau client (société ou personne physique).
...
Nom du cas d'utilisation : Créer un contrat de maintenance
Acteur principal : Administrateur
Objectif : Permettre à l'administrateur de créer un contrat de maintenance et déclencher la génération automatique du planning préventif par le Système.
...
5. Le système démarre une transaction.
6. Le DAO insère le contrat dans la base de données.
...
10. Le DAO insère toutes les interventions générées.
...
Après la mise en place des fonctionnalités de base du système dans le Sprint 1, ce deuxième sprint est consacré à la gestion des interventions préventives et curatives, à la déclaration des pannes et au planning des techniciens. Ce sprint représente le cœur fonctionnel du système SAV.

Les fonctionnalités développées durant ce sprint concernent principalement :
- La consultation du planning en vue hebdomadaire et mensuelle ;
- L'affectation des techniciens avec vérification de disponibilité ;
- Le démarrage et la clôture des interventions par les techniciens ;
- La déclaration de pannes avec pièces jointes multiples par les clients ;
- La prise en charge et la conversion de pannes en interventions curatives ;
- La vérification automatique de la couverture contractuelle à la clôture.
...
6. Le DAO confirme que le technicien n'a pas d'autre intervention à cette date.
7. Le Contrôleur met à jour l'intervention avec le technicien affecté et le statut PLANIFIEE.
8. Le DAO effectue la mise à jour dans la base de données.
...
6. Le DAO effectue la mise à jour.
...
Acteur principal : Administrateur
Objectif : Permettre à l'administrateur d'affecter un technicien disponible à une intervention planifiée.
...
4. Le Contrôleur vérifie que le technicien est actif.
5. Le Contrôleur vérifie la disponibilité du technicien à la date prévue.
6. Le DAO confirme que le technicien est disponible (aucune autre intervention à cette date).
7. Le Contrôleur met à jour l'intervention avec le technicien affecté et le statut PLANIFIEE.
8. Le DAO effectue la mise à jour dans la base de données.
...
1. Le DAO détecte que le technicien a déjà une intervention à cette date.
2. Le Contrôleur retourne un message d'indisponibilité.
3. L'Interface affiche « Technicien indisponible à la date prévue ».

Scénario alternatif 2 — Technicien introuvable ou inactif :
1. Le DAO détecte que le technicien est inexistant ou inactif.
2. L'Interface affiche un message d'erreur.
...
Acteur principal : Technicien
Objectif : Permettre au technicien de valider et clôturer une intervention préventive après sa réalisation.
...
4. Le Contrôleur vérifie que le statut est PLANIFIEE ou EN_COURS.
5. Le Contrôleur met à jour le statut à REALISEE.
6. Le DAO effectue la mise à jour dans la base de données.
...
Acteur principal : Administrateur
Objectif : Permettre à l'administrateur de créer une intervention curative et d'y affecter un technicien disponible.
...
8. Le DAO crée l'intervention curative avec le technicien affecté et le statut PLANIFIEE.
...
1. Le DAO détecte que le technicien est indisponible à la date prévue.
2. L'Interface affiche « Technicien indisponible à la date prévue ».
...
Après la réalisation des fonctionnalités liées à l'authentification, à la gestion du référentiel métier et à la gestion des interventions dans les précédents sprints, ce troisième sprint est consacré aux fonctionnalités de pilotage et de suivi global du système.

Ce sprint permet de finaliser l'application en intégrant :
...
7. Le DAO insère la facture et ses lignes de détail dans la base de données.
8. La base confirme la création.
9. L'Interface affiche la facture générée.

**Scénario alternatif — Intervention non éligible :**
1. L'une des trois conditions n'est pas satisfaite (intervention préventive, ou non réalisée, ou couverte par contrat).
2. Le Contrôleur retourne une erreur.
3. L'Interface affiche « Intervention non éligible à la facturation ».
...
3. Le DAO interroge la base de données avec le filtre approprié.
...
4. Le DAO effectue la mise à jour dans la base.
...
Nom du cas d'utilisation : Consulter les factures
Acteur principal : Administrateur / Client
Objectif : Permettre à l'utilisateur de consulter les factures enregistrées dans le système selon ses droits d'accès.
...
3. Le Contrôleur vérifie le rôle :
   - Si rôle = admin → le DAO récupère toutes les factures de la base.
   - Si rôle = client → le DAO récupère uniquement les factures dont le `clientId` correspond à l'utilisateur connecté.
...
Nom du cas d'utilisation : Consulter l'historique avec filtres
Acteur principal : Administrateur / Technicien / Client
Objectif : Permettre à l'utilisateur de consulter l'historique des interventions selon son rôle et des critères de filtrage.
...
4. Le DAO interroge la base de données avec les filtres appropriés.
...
Nom du cas d'utilisation : Consulter le tableau de bord technicien
Acteur principal : Technicien
Objectif : Permettre au technicien de consulter ses indicateurs personnels.
...
3. Le DAO récupère les interventions assignées au technicien.
...
Sur le plan technique, ce projet a permis de maîtriser une architecture moderne en trois couches (Frontend Next.js/React, Backend API, Base de données Prisma/MySQL) avec une approche DAO (Data Access Object) garantissant la séparation des préoccupations et l'intégrité transactionnelle des données.
