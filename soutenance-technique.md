...
- **Contrats** — `app/api/contracts/route.ts` (195 lignes) : `GET` fait un `findMany` avec `include` (client, équipements, `_count.interventions`), calcule `statut` dynamiquement (`ACTIF`/`BIENTOT_EXPIRE`/`EXPIRE`) via une fonction `computeContractStatut(dateFin)`. `POST` valide dates/clientId/équipements, génère la référence `CTR-{année}-{seq}`, puis dans une transaction crée le contrat **et** génère automatiquement les interventions préventives.
